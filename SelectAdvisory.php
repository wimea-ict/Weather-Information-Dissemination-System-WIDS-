...
//  ." order BY advisory.record_id DESC LIMIT 1";
	...
	default:
		...
		try{
			if($counter >= 5){
				if($language == "english"){
					logFile("HERE inside".$counter);
					$strToDisp = "No Advisory Foundhehe........";
...
public function selectforecastQueryStrings($strType,$SessionId, $PhoneNumber,$language){
    ...
    switch ($strType){
		...
		case    "Daily":
		if($language=="english"){
			$queryStr = " SELECT daily_forecast.weather as description,daily_forecast.id,daily_forecast.time as time, daily_forecast.advisory, daily_forecast.date, daily_forecast.time, "
			." ussdtransaction.Msisdn as telephone, ussdtransaction.regionid as regionid, ussdtransaction.subregionid as subRegion, "
			."ussdregions.name as regionname, ussdsubregions.subregionname as subregionname FROM "

			." daily_forecast LEFT OUTER JOIN ussdtransaction on daily_forecast.region = ussdtransaction.regionid"
			." LEFT OUTER JOIN dailyforecast_region on ussdtransaction.regionid = dailyforecast_region.DRid "
			...
			logFile("This is query in q ".$queryStr);
		}else if($language=="luganda"){
