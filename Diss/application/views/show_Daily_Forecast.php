...
?>
 ...
 <?php   }else{
    ...
    if($lang=='English'){
        ...
        $sqlz = "SELECT season,descriptionLuganda,impactLuganda,graph,audio,issuetime FROM  seasonal_forecast WHERE region = '$region' AND subregionid = '$subregion' AND descriptionLuganda IS NOT NULL order by issuetime DESC limit 1 ";
        $pick1 = $this->db->query($sqlz, $dataz);
        $reg2 = $this->db->get_where('region',array('id'=>$region));
        $reg3 = $this->db->get_where('ussdsubregions',array('subregionid'=>$subregion));
