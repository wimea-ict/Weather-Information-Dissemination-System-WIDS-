...
class Advisory extends CI_Controller
{
    ...
    public function create()
    {
       $data = array(
           'error' => '',
            'button' => 'Create',
            'action' => site_url('index.php/Advisory/create_action'),
            'id' => set_value('id'),
            'type' => set_value('type'),
            'msg' => set_value('msg'),
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($upload == "OK") {
                  ...
                  $all = $this->input->post('timeframe', TRUE);

                  $all .="<br/>"; //time frame
                  $adv = $this->input->post('msg', TRUE);
                  $all .= $adv."<br/>"; //adding the mesage.
                ...
                $data = array(
                    ...
                    'msg' => $all,
                    'file1' => 'food_agric/' . $image,
                );

                $ww = $this->Advisory_model->insert($data);
                $advisory = $this->Advisory_model->get_all();
   ...
   public function _rules()
    {
	$this->form_validation->set_rules('msg', 'Advisory message', 'trim|required');
