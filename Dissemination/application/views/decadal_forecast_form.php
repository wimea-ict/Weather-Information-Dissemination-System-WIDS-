...
<section class="content" id="dashboard-content">
        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data"><table class='table table-bordered'>
           ...
           echo combo_function('region_id','region','name','id','region_id');
                 ...
                 echo "<input type='text' class='form-control' name='regi' id='regi' value= ' ".$p['name']."' disabled />";
                 }
                   }
           ?>
        </td></tr>
        <tr><td>Sub Region <?php echo form_error('sub_region') ?></td>
                <td> <select name="sub_region" class="form-control"><?php //echo combo_function('subregionid','ussdsubregions','subregionname','subregionid','subregionid')
                      $mysql = "SELECT * FROM ussdsubregions";
                      $mysql2 = $this->db->query($mysql);
                      foreach ($mysql2->result_array() as $rows) { ?>
                        <option value="<?php echo $rows['subregionid']; ?>"><?php echo $rows['subregionname'] ?></option>
                    <?php } ?>
